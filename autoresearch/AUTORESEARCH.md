...
### A.2 写 kernel / test / perf

在 `autoresearch/workspace/` 下放三个文件。命名硬约束见 [参考 §2](#2-命名契约)。
...
```text
triton:  median=X.XXms
cann:    median=Y.YYms
```

autoresearch 用正则匹配 `triton|cann ... median=` 抓这两行，分别作为 `gen_time` 和 `base_time`。脚本内部自己负责 warmup、repeats、计时（参考 `dsa_triton_ascend/perf_sfa_triton.py` 的 `run_timing()`）。

最小可跑示例（relu，仅供首次跑通参考）：

<details><summary>relu_kernel.py / relu_test.py / relu_perf.py 示例</summary>
...
```python
# workspace/relu_perf.py
import time, torch, torch.nn.functional as F
from relu_kernel import ModelNew

def run_timing(fn, *args, warmup=5, repeats=20):
...
if __name__ == "__main__":
    ...
    m = ModelNew()
    cann_ms = run_timing(F.relu, x)
    triton_ms = run_timing(m, x)
    print(f"triton:  median={triton_ms:.2f}ms")
    print(f"cann:    median={cann_ms:.2f}ms")
```
</details>
...
```text
/autoresearch --kernel workspace/<op>_kernel.py \
  --test workspace/<op>_test.py \
  --perf workspace/<op>_perf.py \
...
### B.4 [本机] 写 kernel / test / perf

同 [A.2](#a2-写-kernel--test--perf)。
...
`autoresearch/scripts/batch/` 对一批 `(kernel.py, test.py, perf.py)` 任务循环跑 `/autoresearch`。批级状态写 `<batch_dir>/batch_progress.json`，round 级仍在 `ar_tasks/`。
...
# 1. 放 kernel/test/perf（命名必须严格 <op>_kernel.py / <op>_test.py /
#    <op>_perf.py，见参考 §2）
mkdir -p $BATCH_DIR/kernels $BATCH_DIR/tests $BATCH_DIR/perfs
cp workspace/*_kernel.py $BATCH_DIR/kernels/
cp workspace/*_test.py   $BATCH_DIR/tests/
cp workspace/*_perf.py   $BATCH_DIR/perfs/

# 2. Tier-1 预检：纯静态（语法 / import / 必备 export），任意机器可跑、不需要 NPU 或 torch_npu
python scripts/batch/prepare.py $BATCH_DIR

# 3.（可选）Tier-2 预检：在本进程里真跑 test 脚本比对输出
...
| 重新开始 | 删除 `ar_tasks/<task_dir>/`，再 `/autoresearch --kernel ... --test ... --perf ...` |
...
| `--mode` | `kernel-test-perf` / `kernel` | ❌ | `manifest.mode` 或 `kernel-test-perf` | 整批是否要 test/perf；`kernel` 模式下 test/perf 由 agent 现写 |
...
```
INIT
  │  /autoresearch --kernel Y.py --test T.py --perf P.py
  ▼
...
baseline.py / pipeline.py
 └─ task_config.run_eval(..., worker_urls=[...])
     ├─ package_builder.build_package                      # tar.gz: task.yaml + test + perf + kernel + data_files
     └─ HTTP POST /api/v1/run → worker                     # multipart: package + task_id + op_name + timeout
                                  └─ safe_extract →
...
python <repo>/autoresearch/scripts/engine/eval_kernel.py \
    --task-dir . --op-name <op> --kernel-file kernel \
    --test-file <test_file> --perf-file <perf_file> \
    --device-id 0 --phases test,perf --output .eval_result.json
```
...
| 路径 | 用途 |
|---|---|
| `workspace/<op>_kernel.py` / `<op>_test.py` / `<op>_perf.py` | 候选 kernel/test/perf 输入 |
| `task.yaml` | name / arch / editable_files / test_file / perf_file / devices / eval_timeout / max_rounds / metric / `data_files` / `worker.urls` |
...
<batch_dir>/                         ← 批级
  ...
  tests/<op>_test.py                 # 文件名必须严格为 <op>_test.py
  perfs/<op>_perf.py                 # 文件名必须严格为 <op>_perf.py

<repo>/ar_tasks/<op>_<ts>_<uuid>/    ← round 级（由 /autoresearch 维护）
  kernel.py / <test_file> / <perf_file> / task.yaml
...
- 远端 `worker/server.py` 仅暴露 `/api/v1/run`（multipart：tar + task_id + op_name + timeout）与 `/api/v1/status`。新 workflow 不再传 `override_base_us` / `override_base_per_shape_us` 表单字段——每轮 perf 脚本重新测 base，无 sticky baseline 可覆盖。

打包内容（`task_config/package_builder.py`）：`task.yaml` + `test_file` + `perf_file` + `editable_files[*]` + `data_files[*]`。`data_files` 由 scaffold 按 kernel 文件名扫描同目录下名字匹配的数据文件得到，匹配规则见 [§2](#2-命名契约) "同目录数据文件" 一行。
