...
class ModelNew(nn.Module):
    ...
    def _choose_mode(self, x_prefix_shape, idx_prefix_shape, x_gather_dim: int, idx_gather_dim: int) -> str:
    ...
    def forward(self, x: torch.Tensor, index: torch.Tensor, dim: int):
        ...
        if mode == "last_dim":
            y_padded = kernel(x_padded, index_padded)
        else:
            row_map = self._compute_row_map(x_prefix_shape, idx_prefix_shape, index.device)
            y_padded = kernel(x_padded, index_padded, row_map)
